 ...
 * Date        : 4/23/2021
 * Description : Practice using sockets
	...
	while (i < 65536) {
		...
		if ((client_socket = create_socket(&tv)) == -1) {
			...
			if (connect(client_socket, (struct sockaddr *)&serv_addr, sizeof(struct sockaddr_in)) < 0) {
				if (fcntl(client_socket, F_GETFD) >= 0) { //failed to connect to the server so close socket
					close(client_socket);
				}
			} else { //connected to the server
				memset(buf, 0, BUFLEN); // clear buffer
				if ((bytes_recvd = recv(client_socket, buf, BUFLEN-1, 0)) < 0) { //attempt to received characters
