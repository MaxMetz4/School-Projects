...
;; Type signature: (element? item list) -> boolean **DONE**
(define (element? e L)
    (cond((null? L) #F )
         ((equal? (car L) e) #T)
...
;; Type signature: (union list list) -> set **DONE**
(define (union LA LB)
    (if (null? LA) '()
...
(define (intersection LA LB)
  (cond ((equal? (make-set LA) '()) '())
        ((element? (car LA) LB) (make-set (append (list (car LA)) (intersection (cdr LA) LB))))
...
;; Type signature: (subset? set set) -> boolean **DONE**
(define (subset? SA SB)
  (if (null? SA) #t
      (if (element? (car SA) SB) (superset? SB (cdr SA))
          #f)
      )
)
...
;; Type signature: (set-equal? set set) -> boolean **DONE**
...
;; Type signature: (set-difference list list) -> set **DONE**
(define (set-difference LA LB)
  (cond ((equal? (make-set LA) '()) '())
        ((not (member (car (make-set LA)) LB)) (cons (car (make-set LA)) (set-difference (cdr (make-set LA)) LB)))
        (else (set-difference (cdr (make-set LA)) LB))))
...
;; Type signature: (sym-diff list list) -> set **DONE**
...
;; Type signature: (cardinality list) -> int **DONE**
...
;; Type signature: (disjoint? set set) -> boolean **DONE**
...
;; Type signature: (superset? set set) -> boolean **DONE**
...
;; Type signature: (insert element list) -> set **DONE**
(define (insert e L)
  (make-set (append (list e) L))
)
...
;; Type signature: (remove element set) -> set **DONE**
(define (remove e S)
  (cond 
    ((equal? S '()) '())
    ((equal? e (car S)) (cdr S))
