...
;; (CRT-exists? '((1 2) (1 3) (1 5) (1 7) (1 11) (1 13))) -> #t   ALWAYS GIVES F FOR SOME REASON
;;
;; Type Signature: (CRT-exists? cong-sys) -> boolean
(define (CRT-exists? cong-sys)
   (cond
    [(null? cong-sys) #t]
    [(equal? (pairwiseChecker (cdr (moduliCollector cong-sys)) (car (moduliCollector cong-sys))) #f) #f]
    [else (CRT-exists? (cdr cong-sys))]))

(define (moduliCollector cong-sys) "takes the modulus part of all the pairs"
  (cond
    [(null? cong-sys) '()]
    [else (append (list (cadar cong-sys)) (moduliCollector (cdr cong-sys)))]))

(define (pairwiseChecker cong-sys a) "Checks if the list of numbers are pairwise prime"
  (cond
    [(null? cong-sys) #t]
    [(not (equal? (euclid-gcd a (car cong-sys)) 1)) #f]
    [else (pairwiseChecker (cdr cong-sys) a)]))
...
;; (mul-inv 31 76) -> 27
;; (mul-inv 127 555) -> 118
;; (mul-inv 1234 4321) -> -1082 <-- I believe this is wrong, should be 3239

;; Type Signature: (mul-inv int int) -> int
(define (mul-inv a b)
  (mul-inv-helper (modulo a b) b 1))

(define (mul-inv-helper a b q)
  (cond
    [(equal? (modulo (* a q) b) 1) q]
    [else (mul-inv-helper a b (+ q 1))]))
...
(define (m cong-sys)
  (mHelper (moduliCollector cong-sys) 1)) "moduliCollector from CRT-exists"
  
(define (mHelper a b)
  (cond
    [(not (null? a)) (mHelper (cdr a) (* b (car a)))]
    [else b]))
...
(define (CRT-helper cong-sys m)
  (CRT-helper2 cong-sys m 0))

(define (CRT-helper2 cong-sys m sum)
  (cond
    [(not (null? cong-sys)) (CRT-helper2 (cdr cong-sys) m (+ sum (* (* (caar cong-sys) (/ m (cadar cong-sys))) (mul-inv (/ m (cadar cong-sys)) (cadar cong-sys)))))]
    [else sum]))

; Unsure why its not working, for each pair it takes ai*Mi*yi and adds it to sum, returning sum when there are no pairs left
; (caar cong-sys) = ai
; (/ m (cadar cong-sys)) = Mi
; (mul-inv (/ m (cadar cong-sys)) (cadar cong-sys)) = yi
...
(define (CRT cong-sys)
  (cond
    [(eq? (CRT-exists? cong-sys) #f) -1]
    [else (modulo (CRT-helper cong-sys (m cong-sys)) (m cong-sys))])) ; X (mod m)
