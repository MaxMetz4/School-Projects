...
;; (tst1 pz? 40) -> #f
;;
;; Type signature: (pz? peano) -> bool FINISHED
(define (pz? p)
  (if (equal? p z) #t #f))
...
;;
;; Type signature: (succ peano) -> peano FINISHED
(define (succ p)
  (if (list? p) (itp (+ (pti p) 1)) (itp (+ p 1))))
...
;;
;; Type signature: (pred peano) -> peano FINISHED
(define (pred p)
  (if (list? p) (itp (- (pti p) 1)) (itp (- p 1))))
...
;; (tst2 p+ 0 10) -> 10
;;
;; Type signature: (p+ peano peano) -> peano FINISHED
(define (p+ a b)
  (cond [(eq? a '()) b]
        [(eq? b '()) a]
...
;; Type signature: (p- peano peano) -> peano FINISHED
(define (p- a b)
  (cond [(< (pti a) (pti b)) (eopl:error "subtraction error: a < b!")]
        [(eq? a '()) b]
        [(eq? b '()) a]
...
(define (p% a b)
  (cond [(p> b a) a]
        [(p= a b) '()]
        [(equal? b '()) (eopl:error "modulo error: mod is zero!")]
        [else (p% (p- a b) b)]
  )
)
...
(define (p-prime? p)
  (cond [(p= p '()) #f]
        [(p= p '(())) #f]
        [else (helperp-prime '((())) p)]
  )
)
  
(define (helperp-prime a p)
  (cond [(p> (p* a a) p) #t]
        [(p= (p% p a) '()) #f]
        [else (helperp-prime (succ a) p)]
  )
)
