...
(define (mod-exp b e m)
  (cond
    [(equal? e 0) 1] ;Step 1
    [(equal? (modulo e 2) 0) (modulo (expt (modulo (expt b (/ e 2)) m) 2) m)] ;{([b^(e/2) % m] ^ 2) % m}
    [else (modulo (* (modulo (expt b (- e 1)) m) b) m)])) ;{((b^(e-1) % m) * b) % m}
...
(define (encrypt plaintext key)
  (encryptHelper (map char->integer (string->list plaintext)) (car key) (cadr key)))

(define (encryptHelper plaintext m e)
  (cond
    [(null? plaintext) '()]
    [else (append (list(functEc (car plaintext) e m)) (encryptHelper (cdr plaintext) m e))]))

(define (functEc c e m)
  (modulo (expt c e) m))
...
(define (decrypt ciphertext key)
  (list->string (map integer->char (decryptHelper ciphertext (car key) (cadr key)))))

(define (decryptHelper ciphertext m d)
  (cond
    [(null? ciphertext) '()]
    [else (append (list(functDi (car ciphertext) d m)) (decryptHelper (cdr ciphertext) m d))]))

(define (functDi i d m)
  (modulo (expt i d) m))
...
;; m = 247    e = 23          '(159 80 85 138 128 49 80 91 128 201 80 91 154 237 128 86 43 67) p=13 q=19 
;; m = 145    e = 17          '(23 102 45 80 11 11 52 44 115 2 115 104 55 45 96 2 104 11 102 80) p=5 q=29
;; m = 5963   e = 151         '(1080 4133 5460 4133 10 1117 4133 10 3635 4133 5605 5877 4133 1080 5605) p=67 q=89
