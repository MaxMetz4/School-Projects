...
(define (inverse R)
  (if (null? R) '()
  (append (list (list (car(cdr(car R))) (car(car R)))) (inverse (cdr R)))))
...
(define (symmetric-closure R)
  (if (symmetric? R) R (make-set (append R (inverse R)))))
...
(define (relates-to v R)
  (if (not (null? R)) (if (equal? (car(car R)) v) (append (list (car(cdr (car R)))) (relates-to v (cdr R))) (relates-to v (cdr R))) '()))
