...
(define (get-vertices G)
   (cond
    [(null? G) '()]
    [else (usort (append (list (caar G) (cadar G)) (get-vertices (cdr G))))]))

(define (unsorted-get-vertices G)
   (cond
    [(null? G) '()]
    [else (append (list (caar G) (cadar G)) (get-vertices (cdr G)))]))
...
(define (degree G v)
  (cond
    [(equal? '() G) 0]
    [else (degreeHelper G v 0)]))

(define (degreeHelper G v n);n is returned as the degree
  (cond
    [(equal? '() G) n];base case
    [(and (equal? v (caar G)) (equal? v (cadar G))) (+ (degreeHelper (cdr G) v n) 2)];special reflexive edge
    [(or (equal? v (caar G)) (equal? v (cadar G))) (+ (degreeHelper (cdr G) v n) 1)];one vertex detected
    [else (degreeHelper (cdr G) v n)]));no vertex found
...
(define (all-even? G)
  (all-evenHelper G (get-vertices G)))


(define (all-evenHelper G v);preserves original G for reference & v is a list of vertices
  (cond
    [(equal? '() v) #t]
    [(equal? (modulo (degree G (car v)) 2) 1) #f];if vertex is odd, return false
    [else (all-evenHelper G (cdr v))]));otherwise check the next vertex
...
(define (adjacent G v)
  (cond
    [(equal? '() G) '()]
    [(equal? (caar G) v) (usort (append (list (cadar G)) (adjacent (cdr G) v)))];if first # = vertex, return the second number of edge
    [(equal? (cadar G) v) (usort (append (list (caar G)) (adjacent (cdr G) v)))];if second # = vertex, return the first number of edge
...
(define (connected? G)
  (if (equal? '() G) #t (equal? (get-vertices G) (usort (dfs G '() (caar G))))))
; if list is empty, true. Otherwise all the connected verticies should be the same as
; all of the verticies there are in the graph
