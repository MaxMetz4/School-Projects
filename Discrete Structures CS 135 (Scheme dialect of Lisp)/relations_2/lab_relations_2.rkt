...
;; Pledge: I pledge my honor that I have abuded by the Stevens Honor System
;;-------------------------------------------------------------------------------
...
(define (compose RO RI)
  (make-set (composeHelper RO RI RO))
)

(define (composeHelper RO RI iRO)
  (cond [(null? RI) '()]
        [(null? RO) (composeHelper iRO (cdr RI) iRO)]
        [(equal? (cadar RI)(caar RO)) (append(list(list(caar RI)(cadar RO))) (composeHelper (cdr RO) RI iRO))]
        [else (composeHelper (cdr RO) RI iRO)]
  )
)
...
(define (powerHelper R k iR)
  (cond
    [(equal? k 0) '()]
    [(equal? k 1) R]
    [else (make-set (powerHelper (compose R iR) (- k 1) iR))]
  )
)
...
;; may be out of order, but all the pairs should be there
(define (transitive-closure R)
  (if (equal? R '()) '() (make-set(transitive-closureHelper R (counter R))))
)
;; determines size of R
(define (counter R)
  (cond
    [(null? (cdr R)) 1]
    [else (+ 1 (counter (cdr R)))]
  )
)
;; performs power n times and sends back to main method
(define (transitive-closureHelper R n)
  (cond
    [(equal? n 1) R]
    [else (append (transitive-closureHelper R (- n 1)) (power R n))]
  )
)
...
(define (EQ-relation? R n)
  (and (reflexive? R n) (transitive? R) (symmetric? R))
)
