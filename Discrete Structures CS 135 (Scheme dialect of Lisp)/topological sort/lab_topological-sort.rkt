...
(define (indegree Es v)
  (indegreeHelper Es v 0))

(define (indegreeHelper Es v num)
  (cond
    [(null? Es) num]
    [(equal? (cadar Es) v) (indegreeHelper (cdr Es) v (+ num 1))]
    [else (indegreeHelper (cdr Es) v num)]))
...
(define (find-top G)
  (cond
    [(null? (car G)) (tsort-error)]
    [(equal? (indegree (get-edges G) (caar G)) 0) (caar G)]
    [else (find-top (append (list(cdar G)) (list(cadr G))))]))
...
(define (pop-vertex G v)
  (append (list (popv-vertices (car G) v)) (list (popv-edges (cadr G) v))))
...
(define (tsortHelper G sorted)
  (cond
    [(null? (car G)) sorted]
    [else (tsortHelper (pop-vertex G (find-top G)) (append sorted (list (find-top G))))]))
