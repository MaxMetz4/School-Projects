...
let rec type_of_expr : expr -> texpr tea_result = function 
  ...
  | SetRef(e1,e2) ->
    type_of_expr e1 >>= fun x ->
    type_of_expr e2 >>= fun y ->
    (match x with
    | RefType(z) -> if (z=y) then return UnitType else error "setref: type of head and tail do not match"
  ...
  | Cons(h, t) ->
    ...
    if v1=l1 
    then return (ListType(l1))
    else error "cons: type of head and tail do not match"
  | Null(e) ->
    type_of_expr e >>= 
    arg_of_listType "null?: " >>= fun _l1 ->
    return BoolType 
  | Hd(e) ->
    type_of_expr e >>= 
    arg_of_listType "hd: " >>= fun x ->
    return x
  | Tl(e) ->
    type_of_expr e >>= 
  ...
  | Node(de, le, re) ->
    ...
    else error "node: type of data, left tree, and right tree do not match"
  | NullT(t) ->
    type_of_expr t >>= 
    arg_of_treeType "nullt?: " >>= fun _l1 ->
    return BoolType 
  | GetData(t) ->
    type_of_expr t >>= 
    arg_of_treeType "getdata: " >>= fun x ->
    return x  
  | GetLST(t) ->
    type_of_expr t >>= 
