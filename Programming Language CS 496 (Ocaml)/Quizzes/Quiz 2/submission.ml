...
let rec prune n t =
  match t with
  | Node(d,lt,rt) when (height t) < n -> t
  | Node(d,lt,rt) when n = 0 -> Empty
  | Node(d,lt,rt) when n = 1 -> Node(d,Empty,Empty)
...
let rec perfectHelper t height n =
  match t with
  | Node(d,Empty,Empty) -> if height = n then true else false
  | Node(d,lt,rt) -> perfectHelper lt height (n+1) && perfectHelper rt height (n+1)

let perfect t =
  perfectHelper t (height t) 0
