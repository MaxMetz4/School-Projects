...
# Favorite flavor of dorito : the purple one
# *****
...
class Term:
    ...
    def __call__(self, input): # myTerm(input)
        ...
        output = input ** self.exp
        output = output * self.coef
        return output
...
# exponent is strictly decreasing
    ...
    def __add__(self, otherPoly): # addPoly = myPoly + otherPoly
        ...
        Note: If both inputs are properly ordered, so will the outcome! '''
    ...
    def __eq__(self, other): # myPoly == other
        ...
        if isinstance(other, str) == False:
            if (str(self) == str(other)) == True:
                return True
            else:
                return False
    ...
    def __call__(self, input): # myPoly(input)
        ...
        while counter < len(self):
            coef = self.polyList[counter].coef
            exp = self.polyList[counter].exp
            x = Term(coef, exp)
            sum = sum + x(input) #this is where it uses __call__
            counter = counter + 1
        return sum

    # TODO dosent use neg but works
    def __neg__(self): # negPoly = -myPoly
        ...
        while counter < len(self):
            coef = self.polyList[counter].coef
            exp = self.polyList[counter].exp
            x = Term(coef, exp) #creates identical term so the original term isnt modified
            x = -x #uses Term's __neg__
            y.addTerm(x)
            counter = counter + 1
        return str(y)
    ...
    def __sub__(self, other): # subPoly = myPoly - other
        ...
        while counter < len(self):
            if counter == 0:
                coef = self.polyList[counter].coef
                coef2 = other.polyList[counter].coef
                exp = self.polyList[counter].exp
                x = Term(coef-coef2, exp)
                counter = counter + 1
                sum = sum + str(x)
            if counter > 0:
                if (self.polyList[counter].coef < 0):
                    sum = sum + ' - '
                else:
                    sum = sum + ' + '
                coef = self.polyList[counter].coef
