...
private:
	...
	void delete_tree(Node<K, V> *n) {
		// TODO Completed but untested

		//preorder deletion of the tree
	...
	void right_rotate(Node<K, V> *x) {
		...
		//pg 313 in CLRS (swapped left/right from previous left_rotate)
	...
	int height(Node<K, V> *node) const {
		...
		if (node->left == nullptr && node->right == nullptr) {		//base case (current node is a leaf)
			return 0;
		} else {
	...
	size_t leaf_count(Node<K, V> *node) const {
		...
		return (leaf_count(node->left) + leaf_count(node->right));		//add all leaves from both sides of current node
	}
	...
	size_t internal_node_count(Node<K, V> *node) const {
		...
		if (node->left == nullptr && node->right == nullptr) {										//base case (current node is a leaf), leaf does not count as internal
			return 0;
		} else {
			return (internal_node_count(node->left) + internal_node_count(node->right) + 1);		//add all nodes from both sides of current node (+1 represents this node)
		}
	}
	...
	int diameter(Node<K, V> *node) const {
		// TODO completed but untested
		//max lheight + rheight + 1 and max  ldiameter and rdiameter
		if (node == nullptr || root_ == nullptr) {
			return 0;
		}
		//branch = max(diameter(left), diameter(right))
		int branch = (diameter(node->left) > diameter(node->right))?diameter(node->left):diameter(node->right);
		//challenger =  max(branch, height(right) + height(left) + 3) (one for each branch and one for itself)
		int challenger = (height(node->left) + height(node->right) + 3);
		return (branch > challenger)?branch:challenger;
	}
	...
	size_t null_count(Node<K, V> *node) const {
		...
		if (root_ == nullptr) {
			return 1;														//idk does an empty root count as a null?
		}
		if (node == nullptr) {
			return 1;														//idk does an empty root count as a null?
		}
		if (node->left == nullptr && node->right == nullptr) {				//base case there are 2 nulls from each leaf
			return 2;
		} else {
			return (null_count(node->left) + null_count(node->right));		//add all nulls from both sides of current node
		}
	}
	...
	size_t sum_levels(Node<K, V> *node, size_t level) const {
		...
		if (node->left == nullptr && node->right == nullptr) {										//base case at the leaf
			return level;
		} else {
			return (sum_levels(node->left, level+1) + sum_levels(node->right, level+1) + level);	//add the level to the level of descendants, level goes up as it goes down tree
		}
	}
	...
	size_t sum_null_levels(Node<K, V> *node, size_t level) const {
		...
		if (node->left == nullptr && node->right == nullptr) {			//at a leaf, add 2 more nulls at the next level
			return (2 * (level+1));
		} else if (node->left == nullptr) {
			return (sum_null_levels(node->right, level+1) + level+1);	//the left side is a null, add null at level+1 to total and recurse down the right node
		} else if (node->right == nullptr) {
			return (sum_null_levels(node->left, level+1) + level+1);	//the right side is a null, add null at level+1 to total and recurse down the left node
		} else {
			return (sum_null_levels(node->left, level+1) + sum_null_levels(node->right, level+1));	//interior node, go further down tree on both sides
		}
	}
};
